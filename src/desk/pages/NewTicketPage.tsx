...
export default function NewTicketPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const basePath = user.role === 'customer' ? `/desk/portal/tickets/${ticket.id}`
        : user.role === 'agent' ? `/desk/agent/tickets/${ticket.id}`
    ...
    <div className="max-w-2xl mx-auto px-4 py-6 md:px-6 md:py-8">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
              {sources.map((s) => (
                <button key={s.value} type="button" onClick={() => field.onChange(s.value)}
                  className={`flex flex-col items-center gap-1.5 p-3 rounded-xl border-2 text-xs font-medium transition-all ${field.value === s.value ? `${s.color} border-current` : 'border-gray-100 dark:border-gray-800 text-gray-600 dark:text-gray-400 hover:border-gray-300 dark:hover:border-gray-600'}`}>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5">
          ...
          <textarea {...register('summary')} rows={5} placeholder="Describe the conversation, what was discussed, any action items..." className={`${inputCls} resize-none`} />
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5">
          ...
          <p className="text-xs text-gray-400 dark:text-gray-500 mb-4">
            {selectedSource === 'call' ? 'Attach voice recordings (.mp3, .wav)' : selectedSource === 'whatsapp' ? 'Attach screenshots (.jpg, .png)' : 'Attach any relevant files'}
          </p>
        ...
        <div className="flex gap-3 pb-4">
          <button type="button" onClick={() => navigate(-1)} className="flex-1 py-3 px-4 rounded-xl border border-gray-200 dark:border-gray-700 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">Cancel</button>
          <button type="submit" disabled={isSubmitting || uploading} className="flex-1 py-3 px-4 rounded-xl bg-violet-600 hover:bg-violet-700 text-white text-sm font-medium transition-colors disabled:opacity-60 disabled:cursor-not-allowed">
